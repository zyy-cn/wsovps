...
def _string_vec(text: str, dim: int = 64) -> np.ndarray:
...
def _grid_slices(h: int, w: int, gh: int, gw: int) -> list[tuple[slice, slice]]:
    ...
    for yi in range(gh):
        for xi in range(gw):
            out.append((slice(y_edges[yi], y_edges[yi + 1]), slice(x_edges[xi], x_edges[xi + 1])))
    return out


def _extract_patch_features(image: np.ndarray, proj: np.ndarray, gh: int, gw: int) -> list[list[float]]:
    rgb = image.astype(np.float32) / 255.0
    feats: list[list[float]] = []
    for ys, xs in _grid_slices(rgb.shape[0], rgb.shape[1], gh, gw):
        mean_rgb = rgb[ys, xs].reshape(-1, 3).mean(axis=0)
        feats.append(_project(mean_rgb, proj).tolist())
    return feats
...
def _infer_part_map(
    *,
    ...
    support = obj_mask == int(category_id)
    values = part_mask[support]
    values = values[values != 255]
    ids, counts = np.unique(values, return_counts=True)
    ranked = [int(pid) for pid, _ in sorted(zip(ids.tolist(), counts.tolist()), key=lambda t: (-t[1], t[0])) if int(pid) > 0]
    out: dict[str, int] = {}
    for idx, part_key in enumerate(sibling_part_keys):
        if idx < len(ranked):
            out[part_key] = ranked[idx]
    return out
...
def _run() -> dict[str, Any]:
    ...
    for sample in samples:
        ...
        part_mask = np.array(Image.open(sem_seg_file_name))
        ...
        if sum(1 for x in support_mask if x) < 2:
            continue
        patch_features = _extract_patch_features(image, projection, gh, gw)
        sibling_part_id_map = _infer_part_map(
            part_mask=part_mask,
            obj_mask=obj_mask,
            category_id=category_id,
            sibling_part_keys=sibling_part_keys,
        )
        object_text = _project(_string_vec(object_class_key), projection).tolist()
        part_texts = [
        ...
        bundle = FormalInputsBundle(
            ...
            formal_evaluator_output={
                "runtime_payload": {
                    ...
                    "sibling_part_id_map": sibling_part_id_map,
                }
            },
