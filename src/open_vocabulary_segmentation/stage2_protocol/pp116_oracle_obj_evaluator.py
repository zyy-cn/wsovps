...
def _infer_part_id_mapping(
    *,
    part_mask: np.ndarray,
    support_region: np.ndarray,
    sibling_part_keys: list[str],
) -> dict[str, int]:
    support_part = part_mask[support_region]
    support_part = support_part[support_part != 255]
    ids, counts = np.unique(support_part, return_counts=True)
    ranked = [
        int(pid)
        for pid, cnt in sorted(zip(ids.tolist(), counts.tolist()), key=lambda t: (-t[1], t[0]))
        if int(pid) > 0
    ]
    mapping: dict[str, int] = {}
    for idx, part_key in enumerate(sibling_part_keys):
        if idx < len(ranked):
            mapping[part_key] = ranked[idx]
    return mapping
...
def compute_pp116_oracle_obj_runtime_evaluator_output(
    *,
    ...
    explicit_map = _to_int_mapping(runtime_payload.get("sibling_part_id_map"))
    if not explicit_map:
        explicit_map = _infer_part_id_mapping(
            part_mask=part_mask,
            support_region=support_region,
            sibling_part_keys=list(bundle.sibling_part_keys),
        )
    ...
    for part_key, assign_key in zip(bundle.sibling_part_keys, sibling_assignment_keys):
        if part_key not in explicit_map:
            continue
        part_id = explicit_map[part_key]
    ...
    return {
        ...
        "runtime_provenance": {
            ...
            "sibling_part_id_map": explicit_map,
        },
    }
